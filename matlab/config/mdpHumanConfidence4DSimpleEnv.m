function params = mdpHumanConfidence4DSimpleEnv()

%% Grid setup
params.gmin = [-2, -2, 0, 0];
params.gmax = [2, 2, 1, 1];
params.gnums = [20, 20, 15, 15];
params.g = createGrid(params.gmin, params.gmax, params.gnums);
params.extraArgs.g = params.g;
params.bdims = {3,4}; % dimension(s) which contain the belief

%% Joint Dynamics Setup.
params.theta = [1.5,1.5];
params.betas = {0,1.0,0.5};%{0,0.5,1.0}; % Note that first two betas are included in state
...
if params.trueBetaIdx == 1
    ...
    widths = [(params.gmax(1) - params.gmin(1)) + xyoffset;
              ...
              tol;
              (params.gmax(4) - params.gmin(4)) + poffset];
...
params.initial_state = {0,0,1.0/3,1.0/3};
...
params.dyn_sys = MDPHumanConfidence4D(params.initial_state, ...
                                        params.reward_info, ...
                                        params.trueBetaIdx, ...
                                        params.uThresh, ...
                                        gdisc4D, ...
