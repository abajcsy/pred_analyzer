...
% if dist is a scalar, change it to a vector
if length(dist) == 1
    dist = dist*ones(N,1);
end
...
for i = 1:N
    ...
    lines(i, 1:4) = parallelLine(side, dist(i));
end
...
if cleanupLoops
    warning('expandPolygon:cleanupLoops', ...
    'This cleanupLoops has not been updated to deal with multiple dist');
