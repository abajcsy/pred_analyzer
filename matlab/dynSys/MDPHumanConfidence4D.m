classdef MDPHumanConfidence4D < handle
    ...
    methods
        function obj = MDPHumanConfidence4D(z0, reward_info, trueBetaIdx, ...
                                        uThresh, gdisc, gamma, eps, betas, b_range)
            %   Represents joint dynamics of a 2D point human 
            %   and a 3D discrete belief distribution. The belief is over
            ...
            %
            %   State:
            %       z = [x, y, b(beta = beta_1), b(beta = beta_2)]
            %   Dynamics:
            %       zdot = [x + u, 
            %               y + u, 
            %               P(u | x, beta_1)b(beta = b1)/P(x,u),
            %               P(u | x, beta_2)b(beta = b2)/P(x,u)]
            %       where:
        ...
        function beta_posteriors = belief_update(obj,u,z)
            ...
            b2 = obj.pugivenxbeta(u, z, obj.betas{2}) .* z{4};
            b3 = obj.pugivenxbeta(u, z, obj.betas{3}) .* (1-z{3}-z{4});
            normalizer = b1 + b2 + b3;
            
            beta_posteriors = cell(1,2);
            % Is min(max(...)) correct for num beta > 2?
            beta_posteriors{1} = min(max(b1 ./ normalizer, obj.b_range(1)), obj.b_range(2));
            beta_posteriors{2} = min(max(b2 ./ normalizer, obj.b_range(1)), obj.b_range(2));
        end
