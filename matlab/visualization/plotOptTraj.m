...
%             bla = 1;
    ...
    if dims == 4 && numel(bdims) == 1
        ...
        plot3(xcurr(1), xcurr(2), xcurr(bdims{1}), '-o', 'color', color, ...
            'markeredgecolor', color, 'markerfacecolor', color);
    end
    
    % Add first timestamp.
    txt = strcat('t=', num2str(traj_tau(1)), ', b=', num2str(xcurr(bdims{1})));
    tp = text(xcurr(1)+0.05, xcurr(2)+initoffset, xcurr(bdims{1})+0.05, txt);
    tp.Color = color;
    for t=2:length(traj_tau)
        ...
        if dims == 4 && numel(bdims) == 1
            ...
            quiver(xcurr(1), ...
                    ...
                    'markeredgecolor', color, ...
                    'markerfacecolor', color);
%             quiver3(xprev(1),xprev(2),xprev(4), ...
%                     xcurr(1)-xprev(1),xcurr(2)-xprev(2),xcurr(4)-xprev(4),...
%                     'Color', color, ...
%                     'linewidth', 2, ...
%                     'marker', 'o', ...
%                     'markeredgecolor', color, ...
%                     'markerfacecolor', color);
             ...
             p = plot3([xprev(1), xcurr(1)], [xprev(2), xcurr(2)], [xprev(bdims{1}), xcurr(bdims{1})], '-o', ...
                'Color', color, ...
                'markeredgecolor', color, ...
                'markerfacecolor', color);
            p.LineWidth = 2;
        ...
        if t == length(traj_tau)
            txt = strcat('t=', num2str(traj_tau(t)), ', b=', num2str(xcurr(bdims{1})));
            tp = text(xcurr(1)+0.05, xcurr(2)+initoffset+0.1, xcurr(bdims{1})+0.05, txt);
            tp.Color = color;
        end
...
%     end
    ...
    zlim([grid_min(bdims{1}), grid_max(bdims{1})]);
