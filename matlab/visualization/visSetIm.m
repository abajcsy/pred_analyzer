...
switch g.dim
  ...
  case 2
    if isscalar(level)
      %[~, h] = contourf(g.xs{1}, g.xs{2}, data, [level level], 'color', color);
      [~, h] = contour(g.xs{1}, g.xs{2}, data, [level level], 'color', color);
    elseif isempty(level)
      %[~, h] = contourf(g.xs{1}, g.xs{2}, data);
      [~, h] = contour(g.xs{1}, g.xs{2}, data);
    else
      %[~, h] = contourf(g.xs{1}, g.xs{2}, data, level, 'color', color);
...
%% 4D Visualization
function h = visSetIm4D(g, data, color, level, alpha, sliceDim, applyLight)
% h = visSetIm4D(g, data, color, level, sliceDim, applyLight)
...
for i = 1:N
  ...
  % Visualize 3D slices
  h{i} = visSetIm3D(g3D, data3D, color, level, alpha, applyLight);
